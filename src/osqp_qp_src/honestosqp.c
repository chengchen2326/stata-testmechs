...
ST_retcode honestosqp()
{
    ...
    ST_double *x = NULL, obj, max_iter, eps_abs, eps_rel;
    ...
    if ((rc = SF_scal_use("__honestosqp_eps_abs",  &eps_abs)))  goto exit;
    if ((rc = SF_scal_use("__honestosqp_eps_rel",  &eps_rel)))  goto exit;
...
//     printf("\n\n");
    ...
    if (settings) {
        ...
        settings->eps_abs      = SF_is_missing(eps_abs)? 1e-5: eps_abs;
        settings->eps_rel      = SF_is_missing(eps_rel)? 1e-5: eps_rel;
