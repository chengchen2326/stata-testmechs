...
capture program drop honestosqp_plugin
capture program honestosqp_plugin, plugin using("honestosqp_plugin.plugin")

// OSQP Mata wrapper (vendored from HonestDiD; see src/osqp_qp_src/README.md)
...
// -------- OSQP wrapper (vendored from HonestDiD) --------
                          ...
                          | real scalar verbose)
{
    ...
    st_numscalar("__honestosqp_eps_abs",  length(st_numscalar("__honestosqp_eps_abs"))?  st_numscalar("__honestosqp_eps_abs"):  .)
    st_numscalar("__honestosqp_eps_rel",  length(st_numscalar("__honestosqp_eps_rel"))?  st_numscalar("__honestosqp_eps_rel"):  .)
...
void OSQP_cleanup() {
    ...
    st_numscalar("__honestosqp_eps_abs",  J(0, 0, .))
    st_numscalar("__honestosqp_eps_rel",  J(0, 0, .))
...
// Solve QP min 0.5 x' P x + q' x s.t. l <= A x <= u via OSQP.
// Uses our modified OSQP plugin (via osqp.mata wrapper).
// eps_abs = eps_rel = 1e-8 to match R TestMechs settings.
